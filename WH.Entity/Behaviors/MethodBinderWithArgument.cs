...
namespace WH.Entity.Behaviors
{
    ...
    public class MethodBinderWithArgument
    {
        private static readonly ConcurrentDictionary<
            Type,
            ConcurrentDictionary<string, Action<object, object>>
        ...
        public void Invoke(object targetObject, string methodName, object argument)
        {
            ...
            _methodInfo = _targetObjectType
                ...
                .FirstOrDefault(method =>
                {
                    ...
                    var parameterType =
                        parameters[0]?.ParameterType ?? throw new ArgumentException();
                    if (parameterType.IsInterface)
                    {
                        if (
                            _argumentType != null
                            && !_argumentType.GetInterfaces().Contains(parameterType)
                        )
                            return false;
                    }
                    else
                    {
                        if (
                            _argumentType != null
                            && !_argumentType.IsSubclassOf(parameterType)
                            && _argumentType != parameterType
                        )
                            return false;
                    }
            ...
            if (_methodInfo == null)
            {
                throw new ArgumentException(
                    $"{_targetObjectType?.Name} Type's argument {_argumentType?.Name} of method {methodName} inVisible."
                );
            }

            _methodInfo.Invoke(targetObject, new[] { argument });

            var taskArgument = new Tuple<Type, MethodInfo, Type>(
                _targetObjectType,
                _methodInfo,
                    ...
                    var method = Expression
                        .Lambda<Action<object, object>>(
                            Expression.Call(
                                ...
                                Expression.Convert(
                                    paraMessage,
                                    taskArg.Item3 ?? throw new ArgumentException()
                                )
                            ),
                            paraTarget,
                            paraMessage
