        ...
        )
        {
            ...
            switch (engine)
            {
                ...
                case EngineType.OpenCV:
                {
                    ...
                    Cv2.Merge(mats, im);
                    im = CvDnn.BlobFromImage(
                        im,
                        size: new Size(input_size[2], input_size[3]),
                        swapRB: false,
                        crop: false
                    );
                    ...
                    foreach (var name in output_names)
                    {
                        ...
                        Mat result_mat_to_float = new Mat(
                            output_sizes[index][1],
                            output_sizes[index][2],
                            MatType.CV_32F,
                            result_mat.Data
                        );
                        int sum = 1;
                ...
                case EngineType.ONNX:
                {
                    var inputOrtValue = OrtValue.CreateTensorValueFromMemory(
                        input_data,
                        new long[] { input_size[0], input_size[1], input_size[2], input_size[3] }
                    );
                    var inputs = new Dictionary<string, OrtValue>
                    {
                        { input_names[0], inputOrtValue }
                    };
