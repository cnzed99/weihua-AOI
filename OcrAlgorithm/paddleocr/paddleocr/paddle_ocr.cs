...
namespace OpenVinoSharp.Extensions.model.PaddleOCR
{
    public class OCRPredictor
    {
        ...
        public OCRPredictor(
            string det_model = null,
            string cls_model = null,
            string rec_model = null
        )
        {
            if (det_model != null)
            {
            ...
            if (cls)
            {
                ...
                for (int i = 0; i < img_list.Count; i++)
                {
                    if (
                        ...
                        && ocr_result[i].cls_score > ocrCls.m_cls_thresh
                    )
                    {
        ...
        {
            Task<List<OCRPredictResult>>[] tasks = new Task<List<OCRPredictResult>>[img_list.Count];
            ...
            for (int i = 0; i < img_list.Count; i++)
            {
                ...
                tasks[i] = Task.Run(() =>
                {
                    try
                    {
                        return ocr(img_list[j], true, true, true);
                    }
                    finally { }
                });
            }
